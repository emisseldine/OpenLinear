...
#preform least squares on the data
a_hat = np.linalg.lstsq(x,y,rcond=None)[0]
print(a_hat)

# parameters in regression model
a = a_hat[:-1] #np.array([148.7251, -18.8534])
b = a_hat[-1] #54.4017

y_hat = lambda x: a @ x + b

#Evaluate regression model prediction
x1 = np.array([0.846, 1])
y1 = 115
print("y_hat = $" + str(int(round(y_hat(x1),0))*1000))
print("y = $" + str(int(y1*1000)), "\n")

# do it again, but with another sample house
x2 = np.array([1.324, 2])
y2 = 234.50
print("y_hat = $" + str(int(round(y_hat(x2),0))*1000))
print("y = $" + str(int(y1*1000)), "\n")

predicted = a_hat @ x.T

r = D['price'] - predicted                  # vector of predicted errors
print("average = $" + str(round(np.average(r)*1000,2)))
print("root mean square = $" + str(round(np.sqrt(sum(r**2)/len(r))*1000,2)))
print("standard deviation = $" + str(round(np.std(r)*1000,2)), "\n")

de_predicted = predicted-np.average(predicted)  
de_y =  D['price'] - np.average(D['price'])        
print("correlation = " + str(np.inner(de_predicted, de_y )
                             /np.linalg.norm(de_predicted)/np.linalg.norm(de_y)), "\n")

# Graphing
import matplotlib.pyplot as plt
plt.scatter(D['price'], predicted, color='b')
