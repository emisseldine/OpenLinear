...
#Pearson Correlation Coefficients of Attributes
for i in range(len(x)):
    print(f"rho_{i} = {pearson(x[i],y)[0]}")  
print(f"\ncorrelation between x_2 and x_3 = {pearson(x[1],x[2])[0]} \n")
print(f"correlation between x_1 and x_2 = {pearson(x[0],x[1])[0]} \n")    
 
#cross-validate affine models
ns = [2,3,1]
for n in ns: 
    X = np.concatenate([x[:n],[np.ones(m)]]).T
    coeff, train_error, test_error = cross_validate(X, y)
    model = "+".join([str(round(np.mean(coeff[:,i]),4)) + "x_" + str(i+1) for i in range(n+1)])
    print("y = " + model[:-3])
    print(f"train: {np.round(100*np.mean(train_error),2)}%")
    print(f"test: {np.round(100*np.mean(test_error),2)}%\n")

#cross-validate with Booleans
X = np.column_stack((np.ones(m),x[0],x[3],
                     [int(x5 == 2) for x5 in x[4]],
                     [int(x5 == 3) for x5 in x[4]], 
                     [int(x5 == 4) for x5 in x[4]]))
coeff, train_error, test_error = cross_validate(X, y)
c = [round(np.mean(coeff[:,i]),4) for i in range(len(coeff[0]))]
model = f"{c[0]}+{c[1]}x_1+{c[2]}x_3\n"
model += "+".join([f"{c[i]}f_{i-2}(x_5)" for i in range(3,6)])
print("y = " + model)
print(f"train: {np.round(100*np.mean(train_error),2)}%")
print(f"test: {np.round(100*np.mean(test_error),2)}%\n")

#cross-validate with (x_1-a)_+
a = 1.5
X = np.column_stack((np.ones(m),x[0],x[3],
                     [int(x5 == 2) for x5 in x[4]],
                     [int(x5 == 3) for x5 in x[4]], 
                     [int(x5 == 4) for x5 in x[4]],
                     [np.maximum(0, x1-a) for x1 in x[0]]))
coeff, train_error, test_error = cross_validate(X, y)
c = [round(np.mean(coeff[:,i]),4) for i in range(len(coeff[0]))]
model = f"{c[0]}+{c[1]}x_1+{c[2]}x_3\n"
model += "+".join([f"{c[i]}f_{i-2}(x_5)" for i in range(3,6)])
