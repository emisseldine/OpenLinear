...
cyc_diff = lambda n : circulant(np.concatenate(([-1,1], np.zeros(n-2)))).T
...
plt.ion()
plt.plot(np.arange(N), ozone[:N], 'o-')
plt.yscale('log')
plt.ylim(10**(-2.8),10**(-0.8))
plt.show()

#stack of k many 24x24 identity matrices
A = np.concatenate([np.identity(24) for i in range(k)]) 
#24x24 cyclic difference matrix
D = cyc_diff(24) 

#Identifies where there is complete data, as opposed to missing data "NaN"
ind = [k for k in range(N) if ~np.isnan(ozone[k])]

#solve multi-objective least squares problem
x = multi_lstsq([A[ind],D],[ozone[ind],np.zeros(24)], [1,lamb])

#graph solution
plt.plot(np.arange(N), np.concatenate([x for i in range(k)]), 'r')
plt.plot(np.arange(N), ozone[:N], 'o')
plt.yscale('log')
plt.ylim(10**(-2.8),10**(-0.8))
plt.xlabel('lambda = ' + str(lamb))
plt.show()
