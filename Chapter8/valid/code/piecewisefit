...
training_set = np.array([[ -1.4683000758467482, -3.6948838397506325],
...
test_set = np.array([[ 0.16924755009871761, -3.837131660800567],
...
piecewise = lambda x, cusps : np.concatenate(([1,x], [np.maximum(x-a, 0) for a in cusps]))
cusps=[-1,1]
A = np.array([piecewise(x[i], cusps) for i in range(len(x))])
f = np.linalg.lstsq(A,y,rcond=None)[0]
print(f)

print("relative training error (piecewise) =", np.linalg.norm(f[0] + f[1]*x + f[2]*np.maximum(x+1,0) + f[3]*np.maximum(x-1,0)-y)/np.linalg.norm(y)) 
print("relative test error (piecewise) =", np.linalg.norm(f[0] + f[1]*x_test + f[2]*np.maximum(x_test+1,0) + f[3]*np.maximum(x_test-1,0)-y_test)/np.linalg.norm(y_test), "\n") 

f3 = np.polyfit(x,y,3)
print("relative training error (cubic) =", np.linalg.norm(np.poly1d(f3)(x)-y)/np.linalg.norm(y)) 
print("relative test error (cubic) =", np.linalg.norm(np.poly1d(f3)(x_test)-y_test)/np.linalg.norm(y_test), "\n") 
